...
الحصول على موقع نص على الشاشة. سيبحث الأمر عن النص المقدم ويحاول العثور على تطابق بناءً على المنطق الضبابي من [Fuse.js](https://fusejs.io/). هذا يعني أنه إذا قدمت محدد به خطأ مطبعي، أو كان النص الذي تم العثور عليه ليس متطابقًا بنسبة 100٪، فسيظل يحاول إعادة عنصر إليك. انظر [السجلات](#logs) أدناه.
...
يحدد مدى قرب التطابق من الموقع الضبابي (المحدد بالموقع). ستُسجل مطابقة حرف دقيقة بعيدة عن الموقع الضبابي بمسافة تعادل حروف كعدم تطابق كامل. المسافة 0 تتطلب أن يكون التطابق في الموقع المحدد بالضبط. مسافة 1000 ستتطلب تطابقًا مثاليًا ليكون ضمن 800 حرف من الموقع ليتم العثور عليه باستخدام عتبة 0.8.
...
عند أي نقطة تستسلم خوارزمية المطابقة. تتطلب العتبة 0 تطابقًا مثاليًا (للحروف والموقع)، وستطابق العتبة 1.0 أي شيء.
...
سيتم إرجاع التطابقات التي يتجاوز طولها هذه القيمة فقط. (على سبيل المثال، إذا كنت تريد تجاهل تطابقات الحرف الواحد في النتيجة، عيّنها إلى 2)
...
عندما تكون `true`، ستستمر وظيفة المطابقة حتى نهاية نمط البحث حتى إذا تم تحديد تطابق مثالي بالفعل في السلسلة.

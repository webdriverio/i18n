...
Las opciones de servicio son las opciones que se pueden configurar cuando se instancia el servicio y se utilizarán para cada llamada a método.
...
-   **Contextos de Aplicación Soportados:** Web
...
-   **Contextos de Aplicación Soportados:** Web, Aplicación Híbrida (Webview)
...
-   **Contextos de Aplicación Soportados:** Web, Aplicación Híbrida (Webview), Aplicación Nativa

Agrega esquinas de bisel y muesca/isla dinámica a la captura de pantalla para dispositivos iOS.
...
:::info NOTA
Esto solo funcionará cuando [`screenshotPath`](#screenshotpath) se establezca a través de las opciones del plugin, y **NO FUNCIONARÁ** cuando configures las carpetas en los métodos
:::
...
Cuando todas las pruebas se ejecuten, se generará un nuevo archivo JSON con la colección de comparaciones que se puede encontrar en la raíz de tu carpeta `actual`. Los datos se agrupan por:
...
-   `instanceData`, primero el navegador, luego el dispositivo, luego la plataforma
    se verá así
...
Activa/Desactiva el "parpadeo" del cursor en todos los elementos `input`, `textarea`, `[contenteditable]` en la aplicación. Si se establece en `true`, el cursor se configurará como `transparent` antes de tomar una captura de pantalla y se restablecerá cuando termine.
...
Activa/Desactiva todas las animaciones CSS en la aplicación. Si se establece en `true`, todas las animaciones se desactivarán antes de tomar una captura de pantalla y se restablecerán cuando termine.
...
:::info
Al usar esta bandera, cada elemento que contiene texto (no solo `p, h1, h2, h3, h4, h5, h6, span, a, li`, sino también `div|button|..`) recibirá esta propiedad. **NO** hay opción para personalizarlo.
:::
...
-   `mobile`: Esto agregará `_app`, o el nombre del navegador después del `deviceName` para distinguir capturas de pantalla de aplicaciones de las capturas de navegador
...
No puedes proporcionar rutas/carpetas personalizadas en el `formatImageName`. Si deseas cambiar la ruta, consulta cambiar las siguientes opciones:
...
Oculta las barras de desplazamiento en la aplicación. Si se establece en true, todas las barras de desplazamiento se desactivarán antes de tomar una captura de pantalla. Esto está configurado por defecto como `true` para prevenir problemas adicionales.
...
Guardar las imágenes por instancia en una carpeta separada, por ejemplo, todas las capturas de Chrome se guardarán en una carpeta Chrome como `desktop_chrome`.
...
El directorio que contendrá todas las capturas de pantalla actuales/diferentes. Si no se establece, se utilizará el valor predeterminado. También se puede usar una función que devuelva una cadena para establecer el valor screenshotPath:
...
-   **Tipo:** `number`
-   **Obligatorio:** No
-   **Valor predeterminado:** `6` para Android y `15` para iOS (`6` por defecto y `9` se agregará automáticamente para la posible barra de inicio en iPhones con una muesca o iPads que tienen una barra de inicio)
-   **Contextos de Aplicación Soportados:** Web
...
-   **Obligatorio:** No
-   **Valor predeterminado:** `false`
-   **Contextos de Aplicación Soportados:** Web, Aplicación Híbrida (Webview) **Introducido en visual-service@7.0.0**
...
Cuando userBasedFullPageScreenshot se establece en true, el proceso de captura de pantalla simula a un usuario real: desplazándose por la página, capturando capturas de pantalla del tamaño de la ventana y uniéndolas. Este método es útil para páginas con contenido de carga perezosa o renderizado dinámico que depende de la posición de desplazamiento.

Utiliza esta opción si tu página depende de la carga de contenido mientras se desplaza o si deseas preservar el comportamiento de los métodos de captura de pantalla más antiguos.
...
Este módulo también admite dibujar la forma en que un usuario usaría su teclado para _tabular_ a través del sitio web dibujando líneas y puntos de un elemento tabulable a otro.<br/>
El trabajo está inspirado en la publicación del blog de [Viv Richards](https://github.com/vivrichards600) sobre ["AUTOMATIZAR LA TABULABILIDAD DE LA PÁGINA (¿ES ESA UNA PALABRA?) CON PRUEBAS VISUALES"](https://vivrichards.co.uk/accessibility/automating-page-tab-flows-using-visual-testing-and-javascript).<br/>
...
-   **Tipo:** `object`
-   **Obligatorio:** No
-   **Valor predeterminado:** Consulta [aquí](https://github.com/webdriverio/visual-testing/blob/%40wdio/image-comparison-core%401.0.0/packages/image-comparison-core/src/helpers/options.ts#L27-L86) para todos los valores predeterminados
-   **Contextos de Aplicación Soportados:** Web

Las opciones que se pueden cambiar para las líneas y puntos si utilizas los métodos `{save|check}Tabbable`. Las opciones se explican a continuación.
...
Mostrar el número de secuencia de tabulación en el círculo.
...
-   **Tipo:** `object`
-   **Obligatorio:** No
-   **Valor predeterminado:** Consulta [aquí](https://github.com/webdriverio/visual-testing/blob/6a988808c9adc58f58c5a66cd74296ae5c1ad6dc/packages/webdriver-image-comparison/src/helpers/options.ts#L46-L60) para todos los valores predeterminados
-   **Contextos de Aplicación Soportados:** Web, Aplicación Híbrida (Webview), Aplicación Nativa (Consulta [Opciones de comparación de método](./method-options#compare-check-options) para más información)

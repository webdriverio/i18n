...
Protocolo a utilizar cuando se comunica con el servidor del controlador.
...
Puerto en el que se encuentra tu servidor de controlador.
...
Tu nombre de usuario de servicio en la nube (solo funciona para cuentas de [Sauce Labs](https://saucelabs.com), [Browserstack](https://www.browserstack.com), [TestingBot](https://testingbot.com) o [LambdaTest](https://www.lambdatest.com)). Si se establece, WebdriverIO configurará automáticamente las opciones de conexión para ti. Si no utilizas un proveedor de nube, esto se puede utilizar para autenticar cualquier otro backend de WebDriver.
...
Tipo: `String`<br />
Valor predeterminado: `info`<br />
Opciones: `trace` | `debug` | `info` | `warn` | `error` | `silent`

### outputDir

Directorio para almacenar todos los archivos de registro del ejecutor de pruebas (incluidos los registros del informador y los registros `wdio`). Si no se establece, todos los registros se transmiten a `stdout`. Dado que la mayoría de los informadores están hechos para registrar en `stdout`, se recomienda usar esta opción solo para informadores específicos donde tiene más sentido enviar el informe a un archivo (como el informador `junit`, por ejemplo).
...
Te permite usar un agente personalizado `http`/`https`/`http2` [agent](https://www.npmjs.com/package/got#agent) para hacer solicitudes.
...
Tipo: `String`<br />
Valor predeterminado: `process.env.WEBDRIVER_CACHE_DIR || os.tmpdir()`
...
Para un registro más seguro, las expresiones regulares establecidas con `maskingPatterns` pueden ofuscar información sensible del registro.
 - El formato de cadena es una expresión regular con o sin banderas (por ejemplo, `/.../i`) y separadas por comas para múltiples expresiones regulares.
...
Las siguientes opciones (incluidas las enumeradas anteriormente) se pueden usar con WebdriverIO en modo independiente:
...
Si quieres automatizar el navegador utilizando una tecnología de automatización diferente, asegúrate de establecer esta propiedad en una ruta que resuelva a un módulo que se adhiera a la siguiente interfaz:
...
Tipo: `String`<br />
Valor predeterminado: `webdriver`
...
- Si tu parámetro `url` comienza sin un esquema o `/` (como `some/path`), entonces se antepone directamente la `baseUrl` completa.
...
Para aumentar el tiempo de espera de una _prueba_, consulta la documentación del marco.
...
Intervalo predeterminado para que todos los comandos `waitFor*` comprueben si ha cambiado un estado esperado (por ejemplo, visibilidad).
...
Define especificaciones para la ejecución de pruebas. Puedes especificar un patrón glob para hacer coincidir varios archivos a la vez o envolver un glob o conjunto de rutas en una matriz para ejecutarlos dentro de un solo proceso de trabajo. Todas las rutas se consideran relativas a la ruta del archivo de configuración.
...
Excluir especificaciones de la ejecución de pruebas. Todas las rutas se consideran relativas a la ruta del archivo de configuración.
...
Lo mismo que la sección `capabilities` descrita anteriormente, excepto con la opción de especificar un objeto [`multiremote`](/docs/multiremote), o múltiples sesiones de WebDriver en una matriz para ejecución en paralelo.
...
Valor predeterminado: `[{ 'wdio:maxInstances': 5, browserName: 'firefox' }]`

### maxInstances

Número máximo total de trabajadores en ejecución paralela.

__Nota:__ que puede ser un número tan alto como `100`, cuando las pruebas se realizan en algunos proveedores externos como las máquinas de Sauce Labs. Allí, las pruebas no se prueban en una sola máquina, sino en múltiples VMs. Si las pruebas se ejecutarán en una máquina de desarrollo local, usa un número más razonable, como `3`, `4` o `5`. Esencialmente, este es el número de navegadores que se iniciarán simultáneamente y ejecutarán tus pruebas al mismo tiempo, por lo que depende de cuánta RAM haya en tu máquina y cuántas otras aplicaciones se estén ejecutando en tu máquina.
...
Nota: WebdriverIO no maneja la inyección de globales específicos del marco de pruebas.
...
Si quieres que tu ejecución de prueba se detenga después de un número específico de fallas de prueba, usa `bail`.
(Por defecto es `0`, lo que ejecuta todas las pruebas sin importar qué). **Nota:** Una prueba en este contexto son todas las pruebas dentro de un solo archivo de especificación (cuando se usa Mocha o Jasmine) o todos los pasos dentro de un archivo de características (cuando se usa Cucumber). Si quieres controlar el comportamiento de bail dentro de las pruebas de un solo archivo de prueba, echa un vistazo a las opciones disponibles del [marco](frameworks).

Tipo: `Number`<br />
Valor predeterminado: `0` (no abandonar; ejecutar todas las pruebas)
...
Si se establece en `true`, las salidas de registro se agruparán por Especificación de Prueba y se imprimirán solo cuando se complete la Especificación de Prueba.
...
Define el marco de prueba que utilizará el ejecutor de pruebas WDIO.

Tipo: `String`<br />
Valor predeterminado: `mocha`<br />
Opciones: `mocha` | `jasmine`

### mochaOpts, jasmineOpts y cucumberOpts

Opciones específicas relacionadas con el marco. Consulta la documentación del adaptador de marco para conocer las opciones disponibles. Lee más sobre esto en [Frameworks](frameworks).

Tipo: `Object`<br />
Valor predeterminado: `{ timeout: 10000 }`

### cucumberFeaturesWithLineNumbers

Lista de características de cucumber con números de línea (cuando [se usa el marco cucumber](./Frameworks.md#using-cucumber)).
...
Lista de informadores a utilizar. Un informador puede ser una cadena o una matriz de
`['nombreInformador', { /* opciones del informador */}]` donde el primer elemento es una cadena con el nombre del informador y el segundo elemento un objeto con opciones del informador.
...
Determina en qué intervalo el informador debe verificar si están sincronizados si informan sus registros de forma asíncrona (por ejemplo, si los registros se transmiten a un proveedor externo).
...
Determina el tiempo máximo que tienen los informadores para terminar de cargar todos sus registros hasta que el ejecutor de pruebas arroje un error.
...
Una lista de patrones de cadenas que admiten glob que indican al ejecutor de pruebas que observe archivos adicionales, por ejemplo, archivos de aplicación, cuando se ejecuta con la bandera `--watch`. Por defecto, el ejecutor de pruebas ya observa todos los archivos de especificación.
...
Valor predeterminado: `none` si no se proporciona y las pruebas se ejecutan en CI, `new` si no se proporciona, de lo contrario lo que se haya proporcionado
...
Valor predeterminado: almacena archivos de instantáneas en el directorio `__snapshots__` junto al archivo de prueba
...
Tipo: `String`<br />
Valor predeterminado: `null`<br />

## Hooks

El ejecutor de pruebas WDIO te permite establecer hooks que se activan en momentos específicos del ciclo de vida de la prueba. Esto permite acciones personalizadas (por ejemplo, tomar una captura de pantalla si una prueba falla).
...
Se ejecuta antes de que se genere un proceso de trabajo y se puede usar para inicializar servicios específicos para ese trabajador, así como para modificar entornos de ejecución de manera asíncrona.
...
- `specs` (`string[]`): especificaciones para ejecutar en el proceso de trabajo
...
- `retries` (`number`): número de reintentos a nivel de especificación utilizados como se define en [_"Agregar reintentos por archivo de especificación"_](./Retry.md#add-retries-on-a-per-specfile-basis)

### beforeSession

Se ejecuta justo antes de inicializar la sesión de webdriver y el marco de prueba. Te permite manipular configuraciones dependiendo de la capacidad o especificación.
...
Hook que se ejecuta *después* de que finaliza un hook dentro de la suite (por ejemplo, se ejecuta después de llamar a afterEach en Mocha)
...
Función a ejecutar después de que finaliza una prueba (en Mocha/Jasmine).
...
Se ejecuta después de que todos los trabajadores se hayan cerrado y el proceso esté a punto de salir. Un error lanzado en el hook onComplete resultará en la falla de la ejecución de prueba.
...
- `oldSessionId` (`string`): ID de la sesión antigua
- `newSessionId` (`string`): ID de la nueva sesión
...
- `result.error` (`string`): stack de error si el escenario falló

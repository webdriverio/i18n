...
[설정 유형](/docs/setuptypes)(예: 원시 프로토콜 바인딩 사용, 독립 실행형 패키지로서의 WebdriverIO 또는 WDIO 테스트러너)에 따라 환경을 제어하기 위한 다양한 옵션 세트가 제공됩니다.

## WebDriver 옵션

다음 옵션들은 [`webdriver`](https://www.npmjs.com/package/webdriver) 프로토콜 패키지를 사용할 때 정의됩니다:
...
클라우드 서비스 사용자 이름입니다([Sauce Labs](https://saucelabs.com), [Browserstack](https://www.browserstack.com), [TestingBot](https://testingbot.com) 또는 [LambdaTest](https://www.lambdatest.com) 계정에서만 작동합니다). 설정하면 WebdriverIO가 자동으로 연결 옵션을 설정합니다. 클라우드 제공자를 사용하지 않는 경우 다른 WebDriver 백엔드를 인증하는 데 사용할 수 있습니다.
...
클라우드 서비스 액세스 키 또는 비밀 키입니다([Sauce Labs](https://saucelabs.com), [Browserstack](https://www.browserstack.com), [TestingBot](https://testingbot.com) 또는 [LambdaTest](https://www.lambdatest.com) 계정에서만 작동합니다). 설정하면 WebdriverIO가 자동으로 연결 옵션을 설정합니다. 클라우드 제공자를 사용하지 않는 경우 다른 WebDriver 백엔드를 인증하는 데 사용할 수 있습니다.
...
WebDriver 세션에서 실행하려는 기능을 정의합니다. 자세한 내용은 [WebDriver 프로토콜](https://w3c.github.io/webdriver/#capabilities)을 확인하세요. WebDriver 프로토콜을 지원하지 않는 구형 드라이버를 실행하는 경우, 세션을 성공적으로 실행하려면 [JSONWireProtocol capabilities](https://github.com/SeleniumHQ/selenium/wiki/DesiredCapabilities)를 사용해야 합니다.

WebDriver 기반 기능 외에도 원격 브라우저나 장치에 대한 더 깊은 구성을 허용하는 브라우저 및 벤더별 옵션을 적용할 수 있습니다. 이러한 옵션은 해당 벤더 문서에 기록되어 있습니다:
...
추가적으로, Sauce Labs의 [Automated Test Configurator](https://docs.saucelabs.com/basics/platform-configurator/)는 원하는 기능을 클릭하여 이 객체를 생성하는 데 도움이 되는 유용한 도구입니다.
...
모든 테스트러너 로그 파일(리포터 로그 및 `wdio` 로그 포함)을 저장할 디렉토리입니다. 설정하지 않으면 모든 로그가 `stdout`으로 스트리밍됩니다. 대부분의 리포터는 `stdout`에 로그를 출력하도록 설계되어 있으므로, 파일로 보고서를 내보내는 것이 더 합리적인 특정 리포터(예: `junit` 리포터)에서만 이 옵션을 사용하는 것이 좋습니다.
...
드라이버나 그리드에 대한 WebDriver 요청의 타임아웃 시간입니다.
...
요청을 보낼 때 사용할 사용자 정의 `http`/`https`/`http2` [agent](https://www.npmjs.com/package/got#agent)를 지정할 수 있습니다.
...
WebDriver 요청을 보내기 전에 [HTTP 요청 옵션](https://github.com/sindresorhus/got#options)을 가로채는 함수입니다.

타입: `(RequestOptions) => RequestOptions`<br />
기본값: *없음*
...
타입: `(Response, RequestOptions) => Response`<br />
기본값: *없음*

### strictSSL

SSL 인증서가 유효한지 확인할 필요가 없는지 여부입니다.
...
플래그가 활성화되어 있지만 응답에 적절한 키가 없는 경우 아무 작업도 수행하지 않습니다.
...
캐시 디렉토리의 루트 경로입니다. 이 디렉토리는 세션을 시작할 때 다운로드되는 모든 드라이버를 저장하는 데 사용됩니다.
...
더 안전한 로깅을 위해 `maskingPatterns`로 설정된 정규 표현식은 로그에서 민감한 정보를 가릴 수 있습니다.
...
export default class YourAutomationLibrary {
     ...
     * @param {Function} hook 함수에서 클라이언트가 반환되기 전에 수정할 수 있는 hook
     * @param {PropertyDescriptorMap} userPrototype 사용자가 사용자 정의 프로토콜 명령을 추가할 수 있도록 합니다
     * @param {Function} customCommandWrapper 명령 실행을 수정할 수 있습니다
...
- `url` 매개변수가 `/`로 시작하면 `baseUrl`이 앞에 붙습니다(경로가 있는 경우 `baseUrl` 경로 제외).
- `url` 매개변수가 스키마나 `/` 없이 시작하면(예: `some/path`), 전체 `baseUrl`이 직접 앞에 붙습니다.
...
모든 `waitFor*` 명령에 대한 기본 타임아웃입니다. (옵션 이름의 소문자 `f`에 주의하세요.) 이 타임아웃은 `waitFor*`로 시작하는 명령과 그 기본 대기 시간에만 영향을 줍니다.
...
모든 `waitFor*` 명령에 대해 예상 상태(예: 가시성)가 변경되었는지 확인하는 기본 간격입니다.
...
Sauce Labs에서 실행하는 경우, 다른 데이터 센터(US 또는 EU) 간에 테스트를 실행하도록 선택할 수 있습니다.
지역을 EU로 변경하려면 설정에 `region: 'eu'`를 추가하세요.

__참고:__ 이는 Sauce Labs 계정에 연결된 `user` 및 `key` 옵션을 제공한 경우에만 효과가 있습니다.

타입: `String`<br />
기본값: `us`

*(VM 및/또는 에뮬레이터/시뮬레이터에만 해당)*
...
테스트 실행을 위한 스펙을 정의합니다. 한 번에 여러 파일과 일치하는 글롭 패턴을 지정하거나 글롭 또는 경로 집합을 배열로 감싸서 단일 작업자 프로세스 내에서 실행할 수 있습니다. 모든 경로는 구성 파일 경로에서 상대적으로 간주됩니다.
...
위에 설명된 `capabilities` 섹션과 동일하지만, [`multiremote`](/docs/multiremote) 객체를 지정하거나 병렬 실행을 위한 여러 WebDriver 세션을 배열로 지정할 수 있는 옵션이 있습니다.

[위](/docs/configuration#capabilities)에 정의된 것과 동일한 벤더 및 브라우저 특정 기능을 적용할 수 있습니다.
...
__참고:__ Sauce Labs와 같은 외부 벤더에서 테스트가 수행될 때 `100`까지 높은 숫자일 수 있습니다. 여기서 테스트는 단일 머신이 아닌 여러 VM에서 수행됩니다. 테스트가 로컬 개발 머신에서 실행될 경우, `3`, `4` 또는 `5`와 같이 더 합리적인 숫자를 사용하세요. 본질적으로 이는 동시에 시작되어 테스트를 실행하는 브라우저의 수이므로, 머신의 RAM과 실행 중인 다른 앱에 따라 달라집니다.
...
WebdriverIO의 전역 변수(예: `browser`, `$`, `$$`)를 전역 환경에 삽입합니다.
...
(기본값은 `0`으로, 무슨 일이 있어도 모든 테스트를 실행합니다.) **참고:** 이 컨텍스트에서 테스트는 단일 스펙 파일 내의 모든 테스트(Mocha 또는 Jasmine 사용 시) 또는 기능 파일 내의 모든 단계(Cucumber 사용 시)입니다. 단일 테스트 파일 내의 테스트에서 bail 동작을 제어하려면 사용 가능한 [프레임워크](frameworks) 옵션을 확인하세요.

타입: `Number`<br />
기본값: `0` (bail하지 않음; 모든 테스트 실행)

### specFileRetries

전체적으로 실패한 스펙 파일을 재시도하는 횟수입니다.
...
재시도된 스펙 파일을 즉시 재시도할지 아니면 대기열 끝으로 연기할지 여부입니다.
...
서비스는 사용자가 신경 쓰고 싶지 않은 특정 작업을 대신 처리합니다. 거의 노력 없이 테스트 설정을 향상시킵니다.
...
라인 번호가 있는 Cucumber 기능 목록입니다([Cucumber 프레임워크 사용 시](./Frameworks.md#using-cucumber)).
...
리포터가 로그를 비동기적으로 보고할 때(예: 로그가 제3자 벤더로 스트리밍되는 경우) 동기화되었는지 확인하는 간격을 결정합니다.
...
리포터가 모든 로그 업로드를 완료할 때까지 테스트러너가 오류를 발생시키기 전까지의 최대 시간입니다.
...
테스트러너에게 `--watch` 플래그로 실행할 때 추가로 다른 파일(예: 애플리케이션 파일)을 감시하도록 지시하는 글롭 패턴 문자열 목록입니다. 기본적으로 테스트러너는 이미 모든 스펙 파일을 감시합니다.
...
스냅샷을 업데이트하려면 true로 설정하세요. 이상적으로는 CLI 매개변수의 일부로 사용됩니다. 예: `wdio run wdio.conf.js --s`.
...
타입: `(testPath: string, snapExtension: string) => string`<br />
...
WDIO 테스트러너를 사용하면 테스트 수명 주기의 특정 시점에 트리거되는 훅을 설정할 수 있습니다. 이를 통해 사용자 정의 작업(예: 테스트 실패 시 스크린샷 촬영)을 수행할 수 있습니다.

모든 훅은 수명 주기에 대한 특정 정보(예: 테스트 스위트 또는 테스트에 대한 정보)를 매개변수로 가집니다. 모든 훅 속성에 대한 자세한 내용은 [예제 구성](https://github.com/webdriverio/webdriverio/blob/master/examples/wdio.conf.js#L183-L326)에서 확인할 수 있습니다.
...
- `specs` (`string[]`): 작업자 프로세스에서 실행할 스펙
- `retries` (`number`): [_"스펙 파일별 재시도 추가"_](./Retry.md#add-retries-on-a-per-specfile-basis)에 정의된 대로 사용된 스펙 레벨 재시도 횟수
...
테스트 실행이 시작되기 전에 실행됩니다. 이 시점에서 `browser`와 같은 모든 전역 변수에 액세스할 수 있습니다. 사용자 정의 명령을 정의하기에 완벽한 위치입니다.

매개변수:

- `caps` (`object`): 작업자에서 생성된 세션에 대한 기능 포함
- `specs` (`string[]`): 작업자 프로세스에서 실행할 스펙
- `browser` (`object`): 생성된 브라우저/장치 세션의 인스턴스

### beforeSuite

스위트가 시작되기 전에 실행되는 훅(Mocha/Jasmine에서만 해당)
...
테스트 전에 실행되는 함수(Mocha/Jasmine에서만 해당)
...
- `result.passed` (`Boolean`): 테스트가 통과한 경우 true, 그렇지 않으면 false
- `result.retries` (`Object`): [Mocha 및 Jasmine](./Retry.md#rerun-single-tests-in-jasmine-or-mocha) 및 [Cucumber](./Retry.md#rerunning-in-cucumber)에 정의된 대로 단일 테스트 관련 재시도에 대한 정보(예: `{ attempts: 0, limit: 0 }`), 참조
- `result` (`object`): 훅 결과(`error`, `result`, `duration`, `passed`, `retries` 속성 포함)

### afterSuite

스위트가 끝난 후 실행되는 훅(Mocha/Jasmine에서만 해당)
...
- `config` (`object`): WebdriverIO 구성 객체
- `caps` (`object`): 작업자에서 생성된 세션에 대한 기능 포함
- `result` (`object`): 테스트 결과가 포함된 결과 객체
...
- `world` ([`ITestCaseHookParameter`](https://github.com/cucumber/cucumber-js/blob/ac124f7b2be5fa54d904c7feac077a2657b19440/src/support_code_library_builder/types.ts#L10-L15)): pickle 및 테스트 단계에 대한 정보가 포함된 world 객체
...
- `world` ([`ITestCaseHookParameter`](https://github.com/cucumber/cucumber-js/blob/ac124f7b2be5fa54d904c7feac077a2657b19440/src/support_code_library_builder/types.ts#L10-L15)): pickle 및 테스트 단계에 대한 정보가 포함된 world 객체
- `result` (`object`): 시나리오 결과가 포함된 결과 객체
...
- `step` ([`Pickle.IPickleStep`](https://github.com/cucumber/common/blob/b94ce625967581de78d0fc32d84c35b46aa5a075/messages/jsonschema/Pickle.json#L20-L49)): Cucumber 단계 객체
- `scenario` ([`IPickle`](https://github.com/cucumber/common/blob/b94ce625967581de78d0fc32d84c35b46aa5a075/messages/jsonschema/Pickle.json#L137-L175)): Cucumber 시나리오 객체
- `result`: (`object`): 단계 결과가 포함된 결과 객체
- `result.passed` (`boolean`): 시나리오가 통과한 경우 true
- `result.error` (`string`): 시나리오가 실패한 경우 오류 스택
- `result.duration` (`number`): 시나리오 기간(밀리초)
- `context` (`object`): Cucumber World 객체

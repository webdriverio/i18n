...
- `driver`: alias para `browser` (usado quando executando testes móveis)
- `multiRemoteBrowser`: alias para `browser` ou `driver`, mas apenas definido para sessões [Multiremote](/docs/multiremote)
...
__Nota:__ O WebdriverIO não tem controle sobre frameworks usados (por exemplo, Mocha ou Jasmine) que definem variáveis globais ao inicializar seu ambiente.
...
Trong các bài kiểm thử của bạn, bạn có thể _lưu_ ảnh chụp màn hình hoặc so sánh trạng thái hiển thị hiện tại của ứng dụng đang kiểm thử với một hình ảnh cơ sở. Để làm điều đó, dịch vụ này cung cấp [custom matcher](/docs/api/expect-webdriverio#visual-matcher), cũng như các phương thức _kiểm tra_:
...
Dịch vụ này cung cấp các phương thức `save` và `check`. Nếu bạn chạy các bài kiểm thử của mình lần đầu tiên, bạn **KHÔNG NÊN** kết hợp các phương thức `save` và `compare`, các phương thức `check` sẽ tự động tạo hình ảnh cơ sở cho bạn
...
Khi bạn [vô hiệu hóa việc tự động lưu hình ảnh cơ sở](service-options#autosavebaseline), Promise sẽ bị từ chối với cảnh báo sau.
...
Điều này có nghĩa là ảnh chụp màn hình hiện tại được lưu trong thư mục actual và bạn **cần sao chép thủ công vào thư mục baseline**. Nếu bạn khởi tạo `@wdio/visual-service` với [`autoSaveBaseline: true`](./service-options#autosavebaseline), hình ảnh sẽ tự động được lưu vào thư mục baseline.

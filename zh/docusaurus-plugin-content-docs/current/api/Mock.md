...
模拟对象是表示网络模拟的对象，包含有关与给定`url`和`filterOptions`匹配的请求的信息。可以使用[`mock`](/docs/api/browser/mock)命令获取该对象。
...
自动化测试时**不能**使用此命令。在[自动化协议](/docs/automationProtocols)部分了解更多信息。

:::

你可以在我们的[模拟和监视](/docs/mocksandspies)指南中阅读更多关于在WebdriverIO中模拟请求和响应的信息。
...
| `browser` | `Object` | 用于获取mock对象的[浏览器对象](/docs/api/browser)。 |
| `calls` | `Object[]` | 有关匹配的浏览器请求的信息，包含诸如`url`、`method`、`headers`、`initialPriority`、`referrerPolic`、`statusCode`、`responseHeaders`和`body`等属性 |

## 方法

模拟对象提供了各种命令，列在`mock`部分中，允许用户修改请求或响应的行为。
...
当使用[`respond`](/docs/api/mock/respond)或[`respondOnce`](/docs/api/mock/respondOnce)覆盖网络响应时，会发出此事件。响应在事件回调中传递。
...
当使用[`abort`](/docs/api/mock/abort)或[`abortOnce`](/docs/api/mock/abortOnce)中止网络请求时，会发出此事件。失败在事件回调中传递。
...
当添加新匹配时，在`continue`或`overwrite`之前发出此事件。匹配在事件回调中传递。
...
当网络响应既未被覆盖也未被中断，或者响应已被另一个模拟发送时，会发出此事件。`requestId`在事件回调中传递。
...
在此示例中，`firstMock`首先定义并有一个`respondOnce`调用，所以`secondMock`的响应值不会用于第一个请求，但会用于其余请求。